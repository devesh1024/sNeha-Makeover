...
import { staticGalleryItems } from "@/data/gallery";
...
export const galleryQuery = {
  ...
  queryFn: async (): Promise<GalleryItem[]> => {
    ...
    // Always show real images: use DB rows when available, otherwise fall back
    // to the local asset images bundled with the app.
    const rows = (data ?? []) as GalleryItem[];
    return rows.length > 0 ? rows : staticGalleryItems;
  },
};
